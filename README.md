...
《海盜》模組製作手冊 / Pirate Mod Making Guide / パイレーツ MOD制作ガイド
...
css/style.css       共用樣式（三語共用，深淺色自動切換）
...
- **新增章節** → `js/nav.js` 的 `SLUGS` 加一筆 + 三個語言的 `items` 各加一筆 + 三個 `pages/` 各放一個 HTML
- **樣式** → `css/style.css`，三語共用
